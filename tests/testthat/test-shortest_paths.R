    ...
    sapply(list(NULL, pts_d), function(y) {
      if(!(is.null(y) && pw)) {
        test_that(paste0("spat,distances,", dt, ",", ct, ",", dc, ",", pw, ",", is.null(y), ",", (!is.null(u)) + is.list(u)), {
          r <<- expect_no_error(shortest_paths(tr, pts_o, y, dist_comp = dc, pairwise = pw, contiguity = ct, update_rst = u, distance_type = dt))
        })
          ...
          if(!(is.null(u) && pt == "points")) {
            test_that(paste0("spat,distances,", dt, ",", ct, ",", dc, ",", pw, ",", is.null(y), ",", pr, ",", es, ",", (!is.null(u)) + is.list(u), ",",
              ...
              expect_identical(shortest_paths(tr, pts_o, y, pairwise = pw, contiguity = ct, dist_comp = dc, pre = pr, early_stopping = es, update_rst = u,
                ncores = nc, par_lvl = pl, path_type = pt, distance_type = dt), r)
            })
          }
    ...
    mapply(function(y, y_m) {
      if(!(is.null(y) && pw)) {
        test_that(paste0("matrix,distances,distance_type:", dt, ",contiguity:", ct, ",pairwise:", pw, ",without destinations:", is.null(y), ",u:",
          typeof(u_m)), {
          r <<- expect_no_error(shortest_paths(tr_m, pts_o_m, y_m, extent = c(-180, 180, -90, 90), pairwise = pw, contiguity = ct, update_rst = u_m,
            distance_type = dt))
        })

        test_that(paste0("matrix,distances,distance_type:", dt, ",contiguity:", ct, ",pairwise:", pw, ",without destinations:", is.null(y), ",u:",
          typeof(u_m), ",rst_type"), {
          expect_identical(shortest_paths(tr, pts_o, y, pairwise = pw, contiguity = ct, update_rst = u, distance_type = dt), r)
        })
        ...
        mapply(function(pr, es, nc, pl, pt) {
          test_that(paste0("matrix,distances,distance_type:", dt, ",contiguity:", ct, ",pairwise:", pw, ",without destinations:", is.null(y), ",pre:", pr,
            ...
            expect_identical(shortest_paths(tr_m, pts_o_m, y_m, extent = c(-180, 180, -90, 90), pairwise = pw, contiguity = ct, pre = pr,
              early_stopping = es, update_rst = u_m, ncores = nc, par_lvl = pl, path_type = pt, distance_type = dt), r)
          })
  ...
  sapply(list(NULL, pts_d), function(y) {
    if(!(is.null(y) && pw)) {
      test_that(paste0("spat,output:", o, ",distance_type:", dt, ",contiguity:", ct, ",dist_comp:", dc, ",pairwise:", pw, ",without destinations:",
        ...
        r_geom <<- expect_no_error(terra::geom(r))
        r_values <<- expect_no_error(terra::values(r))
      })
      ...
      mapply(function(pr, es, nc, pt) {
        test_that(paste0("spat,output:", o, ",distance_type:", dt, ",contiguity:", ct, ",dist_comp:", dc, ",pairwise:", pw, ",without destinations:",
          ...
          expect_identical(terra::geom(r_x), r_geom)
          expect_identical(terra::values(r_x), r_values)
        })
      ...
      test_that(paste0("spat,", o, ",", dt, ",", ct, ",", dc, ",", pw, ",", is.null(y), ",>0"), {
        expect_true(all(r_values[["distances"]] > 0))
      })
      test_that(paste0("spat,", o, ",", dt, ",", ct, ",", dc, ",", pw, ",", is.null(y), ",!Inf"), {
        expect_true(all(is.finite(r_values[["distances"]])))
      })
      if(o == "both") {
        test_that(paste0("spat,", o, ",", dt, ",", ct, ",", dc, ",", pw, ",", is.null(y), ",distances"), {
          expect_identical(data.table::as.data.table(r_values), shortest_paths(tr, pts_o, y, dist_comp = dc, pairwise = pw, contiguity = ct,
            distance_type = dt))
        })
      }
    }
...
# lines, matrix
para_comb_outer <- expand.grid(o = c("lines", "both"), dt = c("double", "float", "int", "unsigned short int"), ct = c("queen", "rook"), pw = c(TRUE, FALSE),
  stringsAsFactors = FALSE)
  ...
  mapply(function(y, y_m) {
    if(!(is.null(y) && pw)) {
      test_that(paste0("matrix,output:", o, ",contiguity:", ct, ",pairwise:", pw, ",distance_type:", dt, ",without destinations:",
        is.null(y)), {
        r <<- expect_no_error(shortest_paths(tr_m, pts_o_m, y_m, output = o, extent = c(-180, 180, -90, 90), pairwise = pw, contiguity = ct,
          distance_type = dt))
      })

      test_that(paste0("matrix,output:", o, ",contiguity:", ct, ",pairwise:", pw, ",distance_type:", dt, ",without destinations:",
        is.null(y), ",rst_type"), {
        expect_identical(shortest_paths(tr, pts_o, y, output = o, output_class = "list", pairwise = pw, contiguity = ct, distance_type = dt)[c("attributes",
          "lines")], r[c("attributes", "lines")])
      })
      ...
      mapply(function(pr, es, nc, pt) {
        ...
        is.null(y), ",pre:", pr, ",early_stopping:", es, ",ncores:", nc, ",path_type:", pt), {
          expect_identical(shortest_paths(tr_m, pts_o_m, y_m, output = o, extent = c(-180, 180, -90, 90), pairwise = pw, contiguity = ct, pre = pr,
            early_stopping = es, ncores = nc, path_type = pt, distance_type = dt), r)
        })
      ...
      test_that(paste0("matrix,output:", o, ",contiguity:", ct, ",pairwise:", pw, ",distance_type:", dt, ",without destinations:",
        is.null(y), ",>0"), {
        expect_true(all(r[["attributes"]][["distances"]] > 0))
      })
      test_that(paste0("matrix,output:", o, ",contiguity:", ct, ",pairwise:", pw, ",distance_type:", dt, ",without destinations:",
        is.null(y), ",!Inf"), {
        expect_true(all(is.finite(r[["attributes"]][["distances"]])))
      })
      if(o == "both") {
        test_that(paste0("matrix,output:", o, ",contiguity:", ct, ",pairwise:", pw, ",distance_type:", dt, ",without destinations:",
        is.null(y), ",distances"), {
          expect_identical(data.table::as.data.table(r[["attributes"]]), shortest_paths(tr, pts_o, y, pairwise = pw, contiguity = ct, distance_type = dt))
        })
      }
    }
  }, list(NULL, pts_d), list(NULL, pts_d_m), USE.NAMES = FALSE)
...
mapply(function(op, es) {
  mapply(function(y, y_m) {
    test_that(paste0("spat,transition_function,output:", op, ",early_stopping:", es, ",tr_directed:TRUE"), {
      rts <<- expect_no_error(shortest_paths(tr, pts_o, y, output = op,
        tr_fun = function(d, x1, x2, y1, y2, v1, v2, nc) abs(d + x1 + x2 + y1 %% y2 + v1 + v2 + nc), early_stopping = es))
    })
    test_that(paste0("matrix,transition_function,output:", op, ",without destinations:", is.null(y), ",early_stopping:", es, ",tr_directed:TRUE"), {
      rtm <<- expect_no_error(shortest_paths(tr_m, pts_o_m, y_m, output = op, extent = c(-180, 180, -90, 90),
        tr_fun = function(d, x1, x2, y1, y2, v1, v2, nc) abs(d + x1 + x2 + y1 %% y2 + v1 + v2 + nc), early_stopping = es))
    })
    test_that(paste0("spat,transition_function,output:", op, ",without destinations:", is.null(y), ",early_stopping:", es, ",tr_directed:FALSE"), {
      rfs <<- expect_no_error(shortest_paths(tr, pts_o, y, output = op,
    ...
    test_that(paste0("matrix,transition_function,output:", op, ",without destinations:", is.null(y), ",early_stopping:", es, ",tr_directed:FALSE"), {
      rfm <<- expect_no_error(shortest_paths(tr_m, pts_o_m, y_m, output = op, extent = c(-180, 180, -90, 90),
        tr_fun = function(d, x1, x2, y1, y2, v1, v2, nc) abs(d + x1 + x2 + y1 + y2 + v1 + v2 + nc), tr_directed = FALSE, early_stopping = es))
    })
    if(op == "distances") {
      test_that(paste0("transition_function,output:", op, ",early_stopping:", es, ",tr_directed:TRUE"), {
        expect_identical(rts, rtm)
      })
      test_that(paste0("transition_function,output:", op, ",early_stopping:", es, ",tr_directed:FALSE"), {
        expect_identical(rfs, rfm)
      })
    } else {
      test_that(paste0("transition_function,output:", op, ",early_stopping:", es, ",tr_directed:TRUE"), {
        expect_identical(terra::values(rts), as.data.frame(rtm[["attributes"]]))
      })
      test_that(paste0("transition_function,output:", op, ",early_stopping:", es, ",tr_directed:FALSE"), {
        expect_identical(terra::values(rfs), as.data.frame(rfm[["attributes"]]))
      })
    }
  }, list(NULL, pts_d), list(NULL, pts_d_m), USE.NAMES = FALSE)
