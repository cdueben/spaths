...
// [[Rcpp::export]]
Rcpp::List r_upd_paths_woweights(Rcpp::List& from_to, Rcpp::List& starts_targets, Rcpp::List& coords, const std::size_t n_cells, Rcpp::List& upd_rst_r,
  const bool early_stopping, const bool haversine, const bool queen, const int ncores, const bool pairwise, const bool par_lvl_upd, const bool pre,
  const bool int_path, const bool numeric_weights, const bool double_weights, const bool signed_weights, const bool return_dists, const bool show_progress,
  const int bar_limit, const double radius2, const bool from_to_r) {
        ...
        if(double_weights) {
          std::vector<double> distances (n_dists);
          {
            ...
            if(pre) {
              ...
              upd_paths_wweights(graph_to, graph_weights, n_cells, starts, targets, starting_indices, pairwise, false, early_stopping, ncores, par_lvl_upd,
                upd_rst_c, show_progress, bar_limit, static_paths, upd_paths, distances);
            // without precomputed weights
            } else {
              upd_paths_woweights(graph_to, coords, starts, targets, starting_indices, pairwise, early_stopping, haversine, ncores, par_lvl_upd, upd_rst_c,
                show_progress, bar_limit, static_paths, upd_paths, distances, radius2);
            }
          }
          if(return_dists) {
